...
function getDotsAmount() {
    if (window.innerWidth <= 768) {
        return 1700;  // Reducido de 1000 a 500 para móvil
...
function onResize() {
    ...
    // Actualizar cantidad de puntos al cambiar el tamaño
    dotsAmount = getDotsAmount();
    // Recrear la geometría con la nueva cantidad de puntos
    initializeGeometry();
}

// Función para inicializar la geometría
function initializeGeometry() {
    positions = new Float32Array(dotsAmount * 3);
    sizes = new Float32Array(dotsAmount);
    colorsAttribute = new Float32Array(dotsAmount * 3);
    dotsGeometry = new THREE.Geometry();
    ...
    bufferWrapGeom = new THREE.BufferGeometry();
    attributePositions = new THREE.BufferAttribute(positions, 3);
    bufferWrapGeom.addAttribute('position', attributePositions);
    attributeSizes = new THREE.BufferAttribute(sizes, 1);
    bufferWrapGeom.addAttribute('size', attributeSizes);
    attributeColors = new THREE.BufferAttribute(colorsAttribute, 3);
    bufferWrapGeom.addAttribute('color', attributeColors);
    
    // Actualizar wrap
    scene.remove(wrap);
    wrap = new THREE.Points(bufferWrapGeom, shaderMaterial);
    scene.add(wrap);
}
